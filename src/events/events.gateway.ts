...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket): Promise<void> {
    try {
      ...
      const tokenWithoutBearer = token.replace('Bearer ', '');
      console.log("auth in gatwaeu")
      const decoded = jwt.verify(tokenWithoutBearer, process.env.JWT_SECRET) as { userId: number; email: string }; // Verify token
      client.data.user = decoded; // Store user data in client
      console.log('Decoded JWT in gatewya:', decoded);
    } catch (error) {
      client.disconnect(); // Disconnect the client if authentication fails
      console.error('Connection failed due to:', error.message);
    }
  ...
  async handleMessage(client: Socket, payload: { message: string }) {
    ...
    try {
      await this.notificationService.createNotification(client.data.user.userId, payload.message);
